...
//
// Also reused (#1314) by the public geocode search proxy, where it plays a
// different, weaker role: that caller is typically a direct scripted
// requester who controls its own headers, not a victim's browser an
// attacker can't forge headers for, so there it's only a cheap deterrent
// against casual/naive reuse — the geo_token check alongside it is the real
